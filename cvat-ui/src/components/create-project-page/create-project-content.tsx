...
export default function CreateProjectContent(): JSX.Element {
	...
	const [catalogueFileState, setCatalogueFilesState] = useState({
        catalogueFilesList: [],
        catalogueUploading: false
      });

      const [labelSchemaFileState, setLabelSchemaFilesState] = useState({
        labelSchemaFilesList: [],
        labelSchemaUploading: false
      });

const { catalogueUploading, catalogueFilesList } = catalogueFileState;
const { labelSchemaUploading, labelSchemaFilesList } = labelSchemaFileState;

    const catalogueUploadProps = {
      multiple: true,
      onRemove: (file:File) => {
        setCatalogueFilesState((state:any) => {
          const index = state.catalogueFilesList.indexOf(file);
          const newFileList = state.catalogueFilesList.slice();
          newFileList.splice(index, 1);
          return {
            catalogueFilesList: newFileList
          };
        });
      },
      beforeUpload: (file:File) => {
        setCatalogueFilesState((state:any) => ({
          catalogueFilesList: [...state.catalogueFilesList, file]
        }));
        return false;
      },
      catalogueFilesList
    ...
    const labelSchemaUploadProps = {
        multiple: true,
        onRemove: (file:File) => {
            setLabelSchemaFilesState((state:any) => {
            const index = state.labelSchemaFilesList.indexOf(file);
            const newFileList = state.labelSchemaFilesList.slice();
            newFileList.splice(index, 1);
            return {
                labelSchemaFilesList: newFileList
            };
          });
        },
        beforeUpload: (file:File) => {
            setLabelSchemaFilesState((state:any) => ({
            labelSchemaFilesList: [...state.labelSchemaFilesList, file]
          }));
          return false;
        },
        labelSchemaFilesList
    ...
    const onSumbit = async (): Promise<void> => {
		...
		const { catalogueFilesList } = catalogueFileState;
        const { labelSchemaFilesList } = labelSchemaFileState;
        console.log("catalogueFilesList: ", catalogueFilesList);
        projectData.catalogue_files = catalogueFilesList;
        projectData.labelschema_files = labelSchemaFilesList;
    ...
    return (
      ...
      <div className="ant-row ant-form-item" >
         <div className="ant-col ant-form-item-label"><label htmlFor="name"  title="Name">Label Schema</label></div>
         <Upload.Dragger {...labelSchemaUploadProps}>
...
<div></div>
            ...
            <div className="ant-form ant-form-vertical">
      <div className="ant-row ant-form-item" >
         <div className="ant-col ant-form-item-label"><label htmlFor="name"  title="Name">Catalogue</label></div>
         <Upload.Dragger {...catalogueUploadProps}>
    <p className="ant-upload-drag-icon">
      <InboxOutlined />
    </p>
    <p className="ant-upload-text">Click or drag file to this area to upload</p>
