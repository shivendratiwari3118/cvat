...
export function fetchAnnotationsAsync(): ThunkAction {
    return async (dispatch: ActionCreator<Dispatch>): Promise<void> => {
        try {
            ...
            // const fIdData = states.map(item => item.serverID)
            // const finalIdData = fIdData[fIdData.length - 1];
            // console.log(fIdData, "finalIdData", finalIdData)
            // const pasteDeleteFlag: any = localStorage.getItem("setRemoveFlag")
            // const copyId: any = localStorage.getItem("copyShapeId")
            // const useEffectId = localStorage.getItem("useEffectId")
            // if (finalIdData !== undefined) {
            //     console.log(history, "fetchAnnotationsAsync", states.map(item => item.serverID))
            //     localStorage.setItem("fetchAnnotationsId", states[states.length - 1]?.serverID)
            //     localStorage.setItem("fetchAnnotationsIdFlag", true)
            //     if (useEffectId === "true") {
            //         const payload = {
            //             "paste_delete": true,
            //             "copied_track": "",
            //             "new_track": finalIdData,
            //         }
            //         if (finalIdData > copyId) {
            //             serverProxy.jobs.copyTrackAndPaste(payload).then((res: any) => {
            //                 localStorage.setItem("useEffectId", false)
