...
def label_generator_crop(image_folder, list_of_instance_trackedshape,list_of_instance_labeledtrack, last_frame):    
    ...
    for dict_fin in instance_list:
        ...
        img_path_output=os.path.join(image_folder, str(dict_fin["frame"])+".jpeg")
        frame_list.append(dict_fin["frame"])
        img=get_image_from_vid(frame_list)
        cv2.imwrite(img_path_output,img)
        img_path_input=os.path.join(image_folder, str(dict_fin["frame"])+".jpeg")
        #img_res=cv2.imread(img_path_input)
        h,w,c=img.shape
        new_resolution=200
        new_xmin=round((xmin*new_resolution)/w)
        new_xmax=round((xmax*new_resolution)/w)
        new_ymin=round((ymin*new_resolution)/h)
        new_ymax=round((ymax*new_resolution)/h)
        new_coordinates=new_xmin,new_ymin,new_xmax,new_ymax
        resolution=new_xmax-new_xmin, new_ymax-new_ymin
        frame_num=img_path_input.split("/")[-1].split(".")[0]
        img_paths_dict["frame"]=frame_num
        img_base64=img_to_base64(img_path_input)
        img_paths_dict["img_base64"]=img_base64
        img_paths_dict["coordinates"]=coordinates
        img_paths_dict["new_coordinates"]=new_coordinates
        img_paths_dict["resolution"]=resolution
        img_paths.append(img_paths_dict)
